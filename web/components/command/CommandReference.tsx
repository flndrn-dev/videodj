...
export function CommandReference({ onClose, onSelectCommand }: CommandReferenceProps) {
  ...
  const categories = ['library', 'playlist', 'mixing', 'streaming', 'recording', 'help'] as const
                  ...
                  <div key={cmd.command}>
                    ...
                    {cmd.options && cmd.options.map((opt) => (
                      <button
                        key={opt.label}
                        onClick={() => {
                          onSelectCommand(cmd.command + ' ' + opt.label + ' ')
                          onClose()
                        }}
                        style={{
                          width: '100%', display: 'flex', alignItems: 'baseline', gap: 8,
                          padding: '4px 8px 4px 24px', background: 'transparent', border: 'none',
                          cursor: 'pointer', textAlign: 'left',
                        }}
                        onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,0,0.03)' }}
                        onMouseLeave={e => { e.currentTarget.style.background = 'transparent' }}
                      >
                        <span style={{
                          fontSize: 10, fontFamily: 'var(--font-mono)', fontWeight: 600,
                          color: 'rgba(255,255,0,0.6)', minWidth: 120, flexShrink: 0,
                        }}>
                          {opt.example}
                        </span>
