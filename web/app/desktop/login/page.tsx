...
// Desktop App-only sign-in. Posts to /api/auth/magic-link/desktop, which
// emails a videodj:// link. If this page is somehow opened in a regular
// browser (not Electron), we show a "get the Desktop App" page instead of a
// form so the user can't request an email they won't be able to use.
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 32 }}>
          ...
          <p style={{ color: '#8a8aa5', fontSize: 13, margin: '6px 0 0 0' }}>
            {sent ? 'Check your email' : 'Sign in to the Desktop App'}
          </p>
        </div>

        {isElectron === false ? (
          <NotElectron kind="login" />
        ) : sent ? (
          <SentCard email={email} onReset={() => { setSent(false); setEmail('') }} kind="desktop" />
        ) : (
        ...
        <div style={footerStyle}>
          <p style={{ textAlign: 'center', fontSize: 11, color: '#6a6a85', margin: 0 }}>
            ...
            <a href="/desktop/signup" style={{ color: '#ffff00', textDecoration: 'none' }}>Sign up</a>
          </p>
        </div>
      </div>
    </div>
...
// ---------------------------------------------------------------------------
// Shared sub-components + styles (kept in-file to avoid a new module)
// ---------------------------------------------------------------------------

function SentCard({ email, onReset, kind }: { email: string; onReset: () => void; kind: 'desktop' | 'web' }) {
  return (
    <div style={{ textAlign: 'center' }}>
      <div style={sentIconStyle}>✉</div>
      <p style={{ fontSize: 14, color: '#e8e8f2', margin: '0 0 6px 0' }}>
        {kind === 'desktop' ? 'We sent a Desktop App sign-in link to' : 'We sent a sign-in link to'}
      </p>
      ...
      <p style={{ fontSize: 12, color: '#6a6a85', margin: '0 0 24px 0', lineHeight: 1.6 }}>
        {kind === 'desktop'
          ? 'Click the button in your email. It will open the Desktop App.'
          : 'Click the link in your email to sign in.'}<br />The link expires in 15 minutes.
      </p>
...
function NotElectron({ kind }: { kind: 'login' | 'signup' }) {
  return (
    <div style={{ textAlign: 'center' }}>
      <p style={{ fontSize: 14, color: '#e8e8f2', margin: '0 0 12px 0' }}>
        This {kind === 'login' ? 'sign-in' : 'sign-up'} page is only usable inside the Desktop App.
      </p>
      <p style={{ fontSize: 12, color: '#8a8aa5', margin: '0 0 24px 0', lineHeight: 1.6 }}>
        Download the Desktop App, or sign in through the Web App instead.
      </p>
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
        <a href="https://videodj.studio/download" style={primaryLinkStyle}>Get the Desktop App</a>
        <a href={kind === 'login' ? '/login' : '/signup'} style={secondaryLinkStyle}>Use the Web App</a>
      </div>
    </div>
  )
}
