...
export class AutomixController {
  ...
  private planNext() {
    ...
    if (!this.nextSegment) {
      ...
      let fallback = pickNextTrack(this.library, this.currentSegment.track, this.state)
      if (!fallback && this.state) {
        // All tracks played — reset history to allow repeats, keep last 3
        const keep = new Set<string>()
        if (this.currentSegment.track.id) keep.add(this.currentSegment.track.id)
        this.state.playedIds = keep
        this.usedSegments.clear()
        fallback = pickNextTrack(this.library, this.currentSegment.track, this.state)
      }
      if (fallback) {
