...
export class AudioEngine {
  ...
  connect(element: HTMLMediaElement): boolean {
    ...
    try {
      ...
      // BYPASS: source → gain → destination (no EQ processing)
  ...
  /** Switch to EQ mode: source → eqLow → eqMid → eqHigh → gain → destination */
  private activateEQ() {
    if (this.eqActive || !this.source || !this.gainNode || !this.eqLow || !this.eqMid || !this.eqHigh || !this.audioCtx) return
  ...
  /** Switch back to bypass: source → gain → destination (no EQ) */
