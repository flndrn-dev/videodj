...
export class StreamRecorder {
  ...
  /** Start recording from a MediaStream, calling onData with each chunk */
  start(stream: MediaStream, onData: (data: Blob) => void, timeslice = 1000) {
    this.onDataCallback = onData
    this.chunks = []

    // Try VP8+Opus first (best WebM support), fallback to others
    const mimeTypes = [
